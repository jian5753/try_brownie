...
def main(mainAcc, nonOwner, guyA, guyB):
    ...
    TwoWayArbi.deploy({'from': mainAcc})
    WBNB.deposit({'from': mainAcc, 'value': 2 * 1e18})
    WBNB.approve(TwoWayArbi[0].address, UINT256_MAX, {'from': mainAcc})
    #endregion
    ...
    WBNB.withdraw(WBNB.balanceOf(mainAcc), {'from': mainAcc})
    #endregion
    #region non-owner
    print('\n non-owner \n')
